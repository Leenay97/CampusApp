...
```powershell
npm run deploy:front    # билд + заливка .next + рестарт
npm run deploy:back     # заливка кода server/ + рестарт
npm run deploy:public   # заливка public/ + рестарт
```

Скрипты лежат в `scripts/deploy-*.ps1`. Перед первым запуском впишите в каждом
имя pm2-процесса в переменную `$pm2App` (посмотреть: `pm2 list` на сервере).

Ниже — те же шаги вручную, на случай если нужно что-то сделать отдельно.
...
- Вход настроен по SSH-ключу (`~/.ssh/id_ed25519_easycampus`, алиас в `~/.ssh/config` для `157.22.203.160`) — ssh/scp заходят без пароля. Вход root по паролю на сервере отключён (`/etc/ssh/sshd_config.d/00-hardening.conf`).
